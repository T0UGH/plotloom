...
def test_write_receipt_omits_nested_none_values(tmp_path):
    path = receipt_path(tmp_path, "ep001", "clip-01", "volcengine-seedance", "cgt-123")
    receipt = make_receipt(
        tmp_path,
        candidate_path=None,
        provider_data={
            ...
            "nested": {
                "kept": "value",
                "dropped": None,
                "items": ["first", None, {"kept": 1, "dropped": None}],
            },
        },
        media={
            "url": None,
            "artifacts": [
                {"path": "video.mp4", "checksum": None},
                None,
                ("poster.jpg", None),
            ],
        },
    ...
    data = tomllib.loads(path.read_text(encoding="utf-8"))
    assert "candidate_path" not in data
    assert data["provider_data"] == {
        "request_id": "req-1",
        "nested": {
            "kept": "value",
...
def test_receipt_path_rejects_unsafe_episode_and_clip(tmp_path, episode, clip):
    with pytest.raises(ValueError):
        receipt_path(tmp_path, episode, clip, "volcengine-seedance", "cgt-123")
